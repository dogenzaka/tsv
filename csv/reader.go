...
func (reader *Reader) validDelim(r rune) bool {
	return r != 0 && r != rune(reader.Quote) && r != '\r' && r != '\n' && utf8.ValidRune(r) && r != utf8.RuneError
}
...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	if r.Comma == r.Comment || !r.validDelim(r.Comma) || (r.Comment != 0 && !r.validDelim(r.Comment)) {
...
parseField:
	for {
		...
		if len(line) == 0 || line[0] != r.Quote {
			...
			if !r.LazyQuotes {
				if j := bytes.IndexByte(field, r.Quote); j >= 0 {
					col := pos.col + j
			...
			pos.col += quoteLen
			for {
				i := bytes.IndexByte(line, byte(r.Quote))
				if i >= 0 {
					// Hit next quote.
					...
					case rn == rune(r.Quote):
						// `""` sequence (append quote).
						r.recordBuffer = append(r.recordBuffer, r.Quote)
						line = line[quoteLen:]
						pos.col += quoteLen
					case rn == r.Comma:
